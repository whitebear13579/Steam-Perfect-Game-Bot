...
config_path = os.environ.get('STEAM_BOT_CONFIG_PATH')
if not config_path:
    # dev env default path
    config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'db', 'config.json')

config = loadConfig(config_path, getRootLogger())
if config is None:
    log(4, "Failed to load config, exiting...")
    exit(1)

STATUS_FILE = os.path.join(os.path.dirname(config_path), 'bot_status.lock')
...
async def on_ready():
    ...
    # write status file to flag bot is running
    try:
        with open(STATUS_FILE, 'w') as f:
            f.write('running')
        log(2, "Bot status file created.")
    except Exception as e:
        log(4, f"Failed to create status file: {e}")
